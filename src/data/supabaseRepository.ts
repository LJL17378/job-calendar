...
export async function cloudUpdateCompany(userId: string, company: Company) { if (supabase) report((await supabase.from('companies').upsert({ id: company.id, user_id: userId, name: company.name, website: company.website, color: company.color })).error) }
